...
-- //////////////////////////////////////////////////////
/* SETUP SECTION 
No changes needed here. Just run this section to initialize your database.
*/
...
select s.title, alb.title, art.name, count(*) as play_count
  from plays p
  join songs as s on p.song_id = s.song_id
  join albums as alb on s.album_id = alb.album_id
...
select s.title, p.played_date
  from plays as p
  join songs as s on p.song_id = s.song_id
  where p.listener_id = (
    select listener_id 
      from listeners 
      where name = 'Esperanza'
  )
